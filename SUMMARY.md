# Table of contents

* [Main page](README.md)

## Connection

* [Connection through VPN](connection/connection-through-vpn.md)

## Usage

* [Python/Conda](usage/python-conda.md)
* [Common packages](usage/common-packages.md)
* [Tmux (persistent sessions)](usage/tmux-persistent-sessions.md)
* [Sharing code base](usage/sharing-code-base.md)
* [GPU usage](usage/gpu-usage.md)

## Data transfer

* [To or from the host machine](data-transfer/to-or-from-the-host-machine.md)
* [To or from the Internet](data-transfer/to-or-from-the-internet.md)

## Useful links

* [Linux](useful-links/linux.md)
* [Python](useful-links/python.md)
* [Machine Learning](useful-links/machine-learning/README.md)
  * [Classifiers](useful-links/machine-learning/classifiers.md)
  * [Natural Language Processing](useful-links/machine-learning/natural-language-processing.md)
  * [Unsupervised Learning](useful-links/machine-learning/unsupervised-learning.md)
  * [Data Generation](useful-links/machine-learning/data-generation.md)
  * [Autoencoders](useful-links/machine-learning/autoencoders.md)
* [Evaluating Neural Networks](useful-links/evaluating-neural-networks.md)
* [Recurrent Neural Network](useful-links/untitled.md)

## Administration

* [Introduction](administration/introduction/README.md)
  * [TLJH mamba/pip installations for ALL users](https://tljh.jupyter.org/en/latest/install/custom-server.html#step-3-install-conda-pip-packages-for-all-users)
  * [TLJH: IDLE-CULLER - Disabled TODO need a sane value](https://tljh.jupyter.org/en/latest/topic/idle-culler.html)
  * [Allowing user to log in to your JupyterHub without server user name](administration/introduction/allowing-user-to-log-in-to-your-jupyterhub-without-server-user-name.md)
* [Connect to the machine via SSH](administration/connect-to-the-machine/README.md)
  * [Add SSH Keys](administration/connect-to-the-machine/add-ssh-keys.md)
  * [Port forwarding](administration/connect-to-the-machine/port-forwarding.md)
  * [X forwarding (running software with GUI)](administration/connect-to-the-machine/x-forwarding-running-software-with-gui.md)
  * [Run Jupyter remotely](administration/connect-to-the-machine/run-jupyter-remotely.md)
* [Managing users](administration/managing-users.md)
* [System maintenance](administration/system-maintenance.md)
* [Other tasks](administration/other-tasks.md)
